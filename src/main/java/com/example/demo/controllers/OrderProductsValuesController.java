...
public class OrderProductsValuesController {
    ...
    public ResponseEntity<?> deleteById(@PathVariable("id") Integer id) {
        boolean ok = this.orderProductsValuesService.deleteOrderProducts(id);
        if (ok) {
            return new ResponseEntity<String>("Order Products Values was deleted", HttpStatus.OK);

        } else {
            return new ResponseEntity<String>("Order Products Values not found", HttpStatus.NOT_FOUND);
        }
    }
